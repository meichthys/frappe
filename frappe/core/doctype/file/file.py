...
class File(Document):
    ...
    def validate_attachment_references(self):
        ...
        if self.attached_to_field and SPECIAL_CHAR_PATTERN.search(
    ...
    def validate_remote_file(self):
        ...
        if (
            self.file_url
            and "/files/" in self.file_url
            and self.file_url.startswith(site_url)
        ):
        ...
        with zipfile.ZipFile(zip_path) as z:
            for file in z.filelist:
                ...
                try:
                    ...
                    frappe.throw(
                        _("{0} is a not a valid zip file").format(self.file_name)
                    )
